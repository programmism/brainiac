...
// ImportFunc runs an ingest for a source ("notion"|"slack"|"markdown"), optional target
...
func New(c *core.Core, importFn ImportFunc, principal *core.Principal) *mcp.Server {
	...
	if importFn != nil {
		mcp.AddTool(s, &mcp.Tool{
			Name:        "ingest",
			Description: "Import documents into the memory. source is 'notion', 'slack', or 'markdown'; target is a Notion page URL/id or a path (empty = the whole source). Use this when the user shares a doc link or asks to import.",
		}, ingestTool(importFn))
	}
...
type ingestIn struct {
	Source  string `json:"source" jsonschema:"where to import from: notion, slack, or markdown"`
